...
public class DoctorFunctionalTest 
{
    ...
    private static String URLBASE = "http://localhost:8181/DoctorBasico.web/webresources";
    private static String PATHBOOK = "/doctors";
    ...
    public static Archive<?> createDeployment() {
        ...
        WebArchive war = ShrinkWrap
                // Nombre del Proyecto "DoctorBasico.web" seguido de ".war". Debe ser el mismo nombre del proyecto web que contiene los javascript y los  servicios Rest
                .create(WebArchive.class, "DoctorBasico.web.war")
                // Se agrega la dependencia a la logica con el nombre groupid:artefactid:version (GAV)
                .addAsLibraries(resolver.artifact("co.edu.uniandes.doctorbasico:DoctorBasico.logic:1.0")
                        .resolveAsFiles())
    ...
    public List<DoctorDTO> data = new ArrayList();
    ...
    @Test
    @RunAsClient
    public void t1createDoctor() throws InterruptedException {
        boolean success = false;
        Thread.sleep(2500);
        driver.findElement(By.id("create-doctor")).click();
        Thread.sleep(3000);
        ...
        Thread.sleep(2000);
        List<WebElement> doctors = driver.findElements(By.xpath("//div[contains(@ng-repeat,'record in records')]"));
        for (WebElement doctor : doctors) {
            List<WebElement> captions = doctor.findElements(By.xpath("div[contains(@class, 'col-md-4')]/div[contains(@class, 'caption')]/p"));
    ...
    @Test
    @RunAsClient
    public void t2EditDoctor() throws InterruptedException {
        Thread.sleep(1500);
        boolean success = false;
    ...
    @Test
    @RunAsClient
    public void t3DeleteDoctor() throws InterruptedException, IOException {
        Thread.sleep(1500);
        boolean success = false;
        Client cliente = ClientBuilder.newClient();
        Response response = cliente.target(URLBASE + PATHBOOK)
        ...
        Thread.sleep(1000);
        List<WebElement> doctors = driver.findElements(By.xpath("//div[contains(@ng-repeat,'record in records')]"));
        if (doctors.size() == (listDoctorTest.size()-1)) {
            success = true;
        }
        assertTrue(success);
    }
