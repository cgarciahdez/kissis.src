...
public class AdminFunctionalTest 
{
    ...
    private static String PATHBOOK = "/Admins";
    ...
    public List<AdminDTO> data = new ArrayList();
    ...
    @Test
    @RunAsClient
    public void t1createAdmin() throws InterruptedException {
        boolean success = false;
        Thread.sleep(2500);
        driver.findElement(By.id("create-Admin")).click();
        Thread.sleep(3000);
        ...
        driver.findElement(By.id("save-Admin")).click();
        Thread.sleep(2000);
        List<WebElement> Admins = driver.findElements(By.xpath("//div[contains(@ng-repeat,'record in records')]"));
        for (WebElement Admin : Admins) {
            List<WebElement> captions = Admin.findElements(By.xpath("div[contains(@class, 'col-md-4')]/div[contains(@class, 'caption')]/p"));
    ...
    @Test
    @RunAsClient
    public void t2EditAdmin() throws InterruptedException {
        Thread.sleep(1500);
        boolean success = false;
    ...
    @Test
    @RunAsClient
    public void t3DeleteAdmin() throws InterruptedException, IOException {
        Thread.sleep(1500);
        boolean success = false;
        Client cliente = ClientBuilder.newClient();
        Response response = cliente.target(URLBASE + PATHBOOK)
        ...
        Thread.sleep(1000);
        List<WebElement> Admins = driver.findElements(By.xpath("//div[contains(@ng-repeat,'record in records')]"));
        if (Admins.size() == (listAdminTest.size()-1)) {
            success = true;
        }
        assertTrue(success);
    }
